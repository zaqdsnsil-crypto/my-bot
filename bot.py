...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS view_requests (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id TEXT, post_link TEXT, views_count INTEGER, date TEXT, status TEXT DEFAULT "pending")''')
    conn.commit()
    conn.close()
init_db()
...
# ========== سیستم افزایش بازدید واقعی با Telethon ==========
WAITING_FOR_POST_LINK = "waiting_for_post_link"
WAITING_FOR_VIEWS_COUNT = "waiting_for_views_count"
...
async def real_boost_views(channel, message_id, views_count):
    if not API_ID or not API_HASH:
        return False, "❌ API_ID و API_HASH در Railway تنظیم نشده!"
    
    if views_count > 500:
        return False, "⚠️ حداکثر 500 بازدید در هر بار میتونی بزنی (برای جلوگیری از محدودیت تلگرام)"
    ...
    try:
        ...
        return True, f"✅ {views_count} بازدید واقعی با موفقیت اضافه شد!"
        
    except Exception as e:
        return False, f"❌ خطا: {str(e)[:200]}"

async def boost_post_views_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, text, user_id):
    if context.user_data.get(WAITING_FOR_POST_LINK):
        ...
        context.user_data[WAITING_FOR_VIEWS_COUNT] = True
        await update.message.reply_text("✅ لینک ذخیره شد!\n\n🔢 حالا تعداد بازدید رو وارد کن (حداکثر 500):")
        return True
    
    if context.user_data.get(WAITING_FOR_VIEWS_COUNT):
        try:
            views_count = int(text.strip())
            post_link = context.user_data.get("post_link")
            
            if views_count < 1:
                await update.message.reply_text("❌ حداقل 1 بازدید!")
            elif views_count > 500:
                await update.message.reply_text("❌ حداکثر 500 بازدید در هر بار!\nاگه بیشتر نیاز داری، چند بار جداگانه انجام بده.")
            else:
                await update.message.reply_text(f"🚀 شروع افزایش {views_count} بازدید واقعی...\n⏱️ ممکنه چند ثانیه طول بکشه")
                
                channel, message_id = parse_post_link(post_link)
                if not channel:
                    await update.message.reply_text("❌ لینک نامعتبر! باید مثل https://t.me/ChannelName/123 باشه")
                else:
                    success, result = await real_boost_views(channel, message_id, views_count)
                    
                    if success:
                        conn = sqlite3.connect(DB_PATH)
                        conn.execute("INSERT INTO view_requests (user_id, post_link, views_count, date, status) VALUES (?,?,?,?,?)",
                                   (user_id, post_link, views_count, datetime.now().isoformat(), "completed"))
                        conn.commit()
                        conn.close()
                        
                        add_points(user_id, 5, "view_boost")
                        
                        await update.message.reply_text(
                            f"{result}\n\n"
                            f"🔗 لینک: {post_link}\n"
                            f"👁️ تعداد: {views_count}\n"
                            f"⭐ +۵ امتیاز دریافت کردی!",
        ...
        context.user_data[WAITING_FOR_VIEWS_COUNT] = False
        context.user_data["post_link"] = None
        return True
...
def get_main_keyboard():
    keyboard = [
        ...
        [KeyboardButton("📈 افزایش بازدید"), KeyboardButton("🤖 هوش مصنوعی")],
        [KeyboardButton("🔍 جستجوی اینترنت"), KeyboardButton("🎨 ساخت عکس")],
        [KeyboardButton("✏️ ویرایش عکس"), KeyboardButton("🤖 مدیریت گروه")],
    ]
            ...
            for entity in update.message.entities:
                if entity.type == "mention":
                    ...
                    if mentioned == f"@{bot_username}":
                        is_mentioned = True
                        break
        if text in TOPICS_DB or text in ["📊 آمار من", "🏆 جدول لیگ", "🎁 فروشگاه", "⭐ چالش روزانه", "🔮 فال امروز", "🎮 بازی حدس عدد", "🤖 مدیریت گروه", "🤖 هوش مصنوعی", "🔍 جستجوی اینترنت", "🎨 ساخت عکس", "✏️ ویرایش عکس", "📈 افزایش بازدید"]:
            is_mentioned = True
        if not is_mentioned:
            return
    
    # ========== افزایش بازدید واقعی ==========
    if text == "📈 افزایش بازدید":
        context.user_data[WAITING_FOR_POST_LINK] = True
        await update.message.reply_text(
            "🔗 *افزایش بازدید واقعی پست*\n\n"
            "لینک پست رو بفرست:\n"
            "مثال: `https://t.me/ChannelName/123`\n\n"
            "⚠️ حداکثر 500 بازدید در هر بار\n"
            "✅ ربات باید ادمین کانال باشه",
            parse_mode="Markdown", reply_markup=get_main_keyboard()
        )
        return
    
    boost_handled = await boost_post_views_handler(update, context, text, user_id)
    if boost_handled:
        return
...
async def buy_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        await update.message.reply_text("❌ /buy tit1  یا /buy tit2")
        return
...
def main():
    ...
    logger.info("✅ ربات هوشمند با سیستم افزایش بازدید واقعی روشن شد!")
    app.run_polling()
