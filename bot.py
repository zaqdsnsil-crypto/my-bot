...
async def chat_with_ai(user_id, question):
    ...
    try:
        ...
        payload = {"contents": [{"parts": [{"text": f"به فارسی و مفید جواب بده: {question}"}]}]}
        ...
        if "candidates" in data:
            ...
            return f"❌ خطا: {data.get('error', {}).get('message', 'مشخص نیست')}"
...
def main():
    ...
    logger.info("✅ ربات با هوش مصنوعی واقعی Google Gemini روشن شد!")
    app.run_polling()
