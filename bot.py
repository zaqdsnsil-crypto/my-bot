...
# محدودیت ضد اسپم (هر کاربر ۵ پیام در ۱۰ ثانیه)
...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS memories (
        ...
        user_id TEXT, username TEXT, topic TEXT,
        memory_text TEXT, sentiment TEXT, date TEXT
    )''')
    
    # جدول کاربران (امتیاز، لول، تایتل)
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS user_badges (
        user_id TEXT, badge_name TEXT, date_earned TEXT,
        PRIMARY KEY (user_id, badge_name)
    )''')
    
    # جدول خریدها
    c.execute('''CREATE TABLE IF NOT EXISTS user_purchases (
        user_id TEXT, item_id TEXT, purchase_date TEXT,
    ...
    # جدول فعالیت روزانه برای گزارش
    c.execute('''CREATE TABLE IF NOT EXISTS daily_activity (
        date TEXT, user_id TEXT, action TEXT,
        PRIMARY KEY (date, user_id, action)
    )''')
...
def get_title(level):
    titles = {1: "تازه وارد", 2: "عضو فعال", 3: "عضو ویژه",
              4: "کاربر طلایی", 5: "کاربر الماسی", 6: "افسانه", 7: "اسطوره"}
...
def check_badges(user_id):
    ...
    for badge in badges:
        try:
            c.execute("INSERT INTO user_badges (user_id, badge_name, date_earned) VALUES (?,?,?)",
...
# ========== چالش روزانه ==========
DAILY_QUESTS = [
    {"name": "نویسنده", "desc": "۱ خاطره بنویس", "reward": 20, "action": "memory"},
...
def get_daily_quest(user_id):
    ...
    if result and result[1] == today:
        return {"completed": result[0] == 1, "quest": random.choice(DAILY_QUESTS) if result[0] == 0 else None}
    else:
...
def buy_item(user_id, item_id):
    ...
    if points < SHOP[item_id]["price"]:
        conn.close()
        return False, "امتیاز کافی نیست"
    
    try:
        c.execute("INSERT INTO user_purchases (user_id, item_id, purchase_date) VALUES (?,?,?)",
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in number_games:
        try:
            ...
            if result is None:
                ...
                await update.message.reply_text(f"🎉 آفرین! در {extra} بار حدس زدی! +۱۵ امتیاز")
    ...
    if context.user_data.get("waiting_music"):
        ...
        conn = sqlite3.connect(DB_PATH)
        conn.execute("INSERT INTO music_requests (user_id, song_name, date) VALUES (?,?,?)",
    ...
    if context.user_data.get("waiting_for_memory"):
        ...
        conn = sqlite3.connect(DB_PATH)
        conn.execute("INSERT INTO memories (user_id, username, topic, memory_text, date) VALUES (?,?,?,?,?)",
                     (user_id, username, topic_id, text, datetime.now().isoformat()))
        conn.commit()
        conn.close()
    ...
    if text == "📊 آمار من":
        ...
        await update.message.reply_text(
            ...
            f"🎭 تایتل: {stats[2]}\n"
            f"📝 خاطرات: {stats[3]}\n"
            f"🎖️ مدال‌ها: {', '.join(badges) if badges else 'نداری'}",
    ...
    if text == "⭐ چالش روزانه":
        ...
        if quest["completed"]:
            ...
            await update.message.reply_text(
                ...
                f"برای تایید انجام چالش: /complete_quest",
                parse_mode="Markdown", reply_markup=get_main_keyboard()
            )
        return 0
    ...
    if text == "🤖 مدیریت گروه":
        await update.message.reply_text(
            "🤖 *مدیریت گروه*\n\n"
            f"برای صحبت: منشن کن @{bot_username}\n"
            f"یا از دکمه‌ها استفاده کن",
            parse_mode="Markdown", reply_markup=get_main_keyboard()
        )
        return 0
...
async def buy_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    success, msg = buy_item(str(update.effective_user.id), item_id)
    await update.message.reply_text(msg)

async def complete_quest_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = str(update.effective_user.id)
    if complete_quest(user_id):
        await update.message.reply_text("✅ چالش تکمیل شد! +۲۰ امتیاز")
    else:
        await update.message.reply_text("❌ چالش امروز رو قبلاً انجام دادی یا فعال نیست")
