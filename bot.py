...
# ========== دیتابیس با timeout برای جلوگیری از قفل شدگی ==========
...
async def chat_with_ai(user_id, question):
    ...
    try:
        response = requests.post(
            ...
            json={"model": "google/gemini-2.0-flash-exp:free", "messages": [{"role": "user", "content": f"به فارسی و مفید جواب بده: {question}"}], "max_tokens": 500},
            timeout=30
        )
        data = response.json()
        if "choices" in data:
            ...
            with get_db_connection() as conn:
                conn.execute("INSERT INTO ai_conversations (user_id, question, answer, date) VALUES (?,?,?,?)", (user_id, question, answer, datetime.now().isoformat()))
                conn.commit()
            return answer
        else:
            return f"❌ خطا: {data.get('error', {}).get('message', 'خطای ناشناخته')}"
    ...
    if context.user_data.get(WAITING_FOR_VIEWS_COUNT):
        try:
            ...
            if views_count < 1:
                ...
                await update.message.reply_text(f"🚀 شروع افزایش {views_count} بازدید...\n⚠️ این قابلیت فقط با اکانت شخصی کار میکنه، نه ربات.")
                with get_db_connection() as conn:
                    conn.execute("INSERT INTO view_requests (user_id, post_link, views_count, date, status) VALUES (?,?,?,?,?)", (user_id, post_link, views_count, datetime.now().isoformat(), "pending"))
                    conn.commit()
                add_points(user_id, 5, "view_boost")
                await update.message.reply_text(f"✅ درخواست افزایش {views_count} بازدید ثبت شد!\n⭐ +۵ امتیاز", reply_markup=get_main_keyboard())
        except ValueError:
...
def main():
    ...
    logger.info("✅ ربات هوشمند با دیتابیس پایدار روشن شد!")
    app.run_polling()
