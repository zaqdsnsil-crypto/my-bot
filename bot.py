...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    keyboard = [
        ...
        [InlineKeyboardButton("🏙️🌃 شهری بازی", callback_data="menu_city"),
         InlineKeyboardButton("😢🌧️ روزهای تنها", callback_data="menu_lonely")],
        [InlineKeyboardButton("🎉🎊 سرگرمی", callback_data="menu_fun"),
         InlineKeyboardButton("📖🌸 رمان", callback_data="menu_novel")],
        [InlineKeyboardButton("🤖⚙️ مدیریت گروه", callback_data="menu_admin")],
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    text = (
        "🌟💫🌟 سلام! خوش اومدی 🌟💫🌟\n"
        "━━━━━━━━━━━━━\n"
        "🎀 یه موضوع دلخواهت رو انتخاب کن 🎀\n"
        "━━━━━━━━━━━━━"
    )
...
def main():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            MAIN_MENU: [
                # فقط callback های inline keyboard - به پیام‌های متنی کاری نداره
                CallbackQueryHandler(button_handler)
            ],
        },
        fallbacks=[CommandHandler("start", start)],
        per_chat=True,
        # پیام‌های عادی رو نادیده میگیره
        allow_reentry=True,
    )
