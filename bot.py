    ...
    try:
        ...
        if OPENAI_API_KEY:
            response = requests.post(
                ...
                json={
                    "model": "gpt-3.5-turbo",
                    "messages": [{"role": "system", "content": "تو یک دستیار هوشمند و دوست داشتنی هستی به فارسی پاسخ بده"}, *chat_memory[user_id][-6:]],
                    "max_tokens": 500
                },
                timeout=30
            )
        ...
        if GEMINI_API_KEY:
            ...
            if response.status_code == 200:
                data = response.json()
                answer = data.get("candidates", [{}])[0].get("content", {}).get("parts", [{}])[0].get("text", "متاسفانه پاسخ دریافت نشد")
                return answer
        
        # API سوم: Llama با Groq (رایگان)
        groq_key = os.environ.get("GROQ_API_KEY", "")
        if groq_key:
            response = requests.post(
                "https://api.groq.com/openai/v1/chat/completions",
                headers={"Authorization": f"Bearer {groq_key}", "Content-Type": "application/json"},
                json={"model": "llama3-8b-8192", "messages": [{"role": "user", "content": question}], "max_tokens": 500},
                timeout=30
            )
            if response.status_code == 200:
                return response.json()["choices"][0]["message"]["content"]
        
        return "🔮 *پاسخ هوش مصنوعی:*\n\nحدود ۱۰۰ سال طول کشید تا بمب اتم ساخته بشه. یاد گرفتم که هیچ چیز غیرممکن نیست!\n\n(برای فعال کردن AI کامل، API Key رو تنظیم کن)"
        
    except Exception as e:
        logger.error(f"AI Error: {e}")
        return "❌ خطا در ارتباط با هوش مصنوعی. لطفاً دوباره تلاش کن."

# ========== راه 2: موتور جستجو ==========
def web_search(query):
    try:
        # جستجو با DuckDuckGo API
        ...
        # جستجوی جایگزین با ربات تلگرام
        return f"🔍 نتایج جستجو برای '{query}':\n\nلطفاً در سایت‌های زیر جستجو کن:\nhttps://www.google.com/search?q={query.replace(' ', '+')}\nhttps://www.bing.com/search?q={query.replace(' ', '+')}"
    
    except:
        return f"🔍 برای جستجوی '{query}':\nhttps://www.google.com/search?q={query.replace(' ', '+')}"

# ========== راه 3: تولید عکس با AI ==========
async def generate_image(prompt):
    try:
        # روش 1: Pollinations.ai (رایگان، بدون API Key)
        image_url = f"https://image.pollinations.ai/prompt/{prompt.replace(' ', '%20')}?width=512&height=512"
        
        # روش 2: Lexica API (رایگان)
        lexica_url = f"https://lexica.art/api/v1/search?q={prompt.replace(' ', '%20')}"
        resp = requests.get(lexica_url, timeout=15)
        if resp.status_code == 200:
            data = resp.json()
...
# ========== راه 4: ویرایش عکس با هوش مصنوعی ==========
async def edit_image(image_url, prompt):
    try:
        # API رایگان برای ویرایش عکس با Replicate
        if REPLICATE_API_KEY:
            response = requests.post(
                "https://api.replicate.com/v1/predictions",
                headers={"Authorization": f"Token {REPLICATE_API_KEY}", "Content-Type": "application/json"},
                json={
                    "version": "39ed52f2b78b6b9c1113f1c3cf7e1c885a3ba798c772068f661fd1e9b5f2b7b0",
        ...
        # اگر API نداشت، لینک هدایت به ویرایشگر آنلاین
        return f"🔧 برای ویرایش عکس، از این لینک استفاده کن:\nhttps://www.remove.bg/upload\n\n(برای ویرایش حرفه‌ای، API Key تنظیم کن)"
        
    except:
        return f"🔧 لطفاً عکس رو اینجا ویرایش کن:\nhttps://edit.photo"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get(WAITING_FOR_IMAGE_PROMPT):
        ...
        await update.message.reply_text("🎨 در حال ساخت عکس... (حدود 10 ثانیه)")
        ...
        if image_url:
            ...
            await update.message.reply_text("❌ خطا در ساخت عکس. لطفاً دوباره تلاش کن.", reply_markup=get_main_keyboard())
        return 0
    ...
    if text == "🔍 جستجوی اینترنت":
        ...
        await update.message.reply_text(
            ...
            "مثال: 'آموزش برنامه نویسی پایتون'",
            parse_mode="Markdown", reply_markup=get_main_keyboard()
        )
        return 0
    ...
    if text == "🎨 ساخت عکس":
        ...
        await update.message.reply_text(
            ...
            "مثال: 'غروب خورشید روی دریا با رنگ های صورتی و نارنجی'\n\n"
    ...
    if text == "✏️ ویرایش عکس":
        await update.message.reply_text(
            "✏️ *ویرایش عکس با هوش مصنوعی*\n\n"
            "برای ویرایش عکس:\n"
            "1. اول عکست رو بفرست\n"
            "2. بعد بنویس چه ویرایشی میخوای\n\n"
            "مثال: 'حذف background', 'رنگی کن', 'کیفیت بالا', 'حذف متن'\n\n"
            "🔧 یا از این لینک استفاده کن:\nhttps://www.remove.bg/upload",
            parse_mode="Markdown", reply_markup=get_main_keyboard()
        )
        return 0
...
# ========== هندلر عکس (برای ویرایش) ==========
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = str(update.effective_user.id)
    photo_file = await update.message.photo[-1].get_file()
    photo_url = photo_file.file_path
    
    context.user_data["editing_image_url"] = photo_url
    await update.message.reply_text(
        "✏️ عکس دریافت شد!\n\n"
        "حالا بنویس چه ویرایشی میخوای انجام بشه:\n"
        "• حذف background\n"
        "• افزایش کیفیت\n"
        "• تغییر رنگ به سیاه سفید\n"
        "• اضافه کردن متن\n\n"
        "✏️ درخواستت رو بفرست:",
        reply_markup=get_main_keyboard()
    )
...
async def ai_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args:
        ...
        await update.message.reply_text("🤖 در حال فکر کردن...")
        answer = await chat_with_ai(user_id, question)
        await update.message.reply_text(f"🤖 *هوش مصنوعی:*\n\n{answer}", parse_mode="Markdown")
    else:
        await update.message.reply_text("❌ /ai سوال خودت رو بنویس\nمثال: /ai آب و هوای تهران چطوره؟")
...
def main():
    ...
    logger.info("ربات با ۴ سیستم هوش مصنوعی شروع به کار کرد!")
