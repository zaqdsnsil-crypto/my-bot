...
# ========== هندلر اصلی پیام‌ها با سکوت در گروه ==========
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = update.message.text
    user_id = str(update.effective_user.id)
    username = update.effective_user.username or update.effective_user.first_name
    
    # ========== بررسی: آیا ربات منشن شده؟ (فقط برای گروه) ==========
    ...
    if update.message.chat.type in ["group", "supergroup"]:
        if update.message.entities:
            for entity in update.message.entities:
                if entity.type == "mention":
                    ...
                    if mention_start <= len(text):
                        mentioned = text[mention_start:mention_end]
        ...
        # اگه متن با / شروع بشه (دستور)
        if text.startswith("/"):
            is_mentioned = True
        
        # اگر کاربر روی دکمه کلیک کرده (از منو)
        if text in TOPICS_DB or text in ["📊 آمار من", "🤖 مدیریت گروه", "🎵 جستجوی آهنگ"]:
            is_mentioned = True
        
        # اگر منشن نشده و دکمه نزده => سکوت کن (برگرد بدون جواب)
        if not is_mentioned:
            return 0
    
    # ========== ادامه کد فقط برای پیوی یا منشن شده ==========
    ...
    if text == "🤖 مدیریت گروه":
        await update.message.reply_text(
            "🤖 *مدیریت گروه*\n\n"
            "✅ برای صحبت با ربات توی گروه:\n"
            "👉 اول منشن کن @{}\n"
            "👉 یا از دکمه‌های پایین استفاده کن\n\n"
            "✅ توی پیوی (Private) میتونی مستقیم باهاش حرف بزنی".format(bot_username),
            parse_mode="Markdown",
            reply_markup=get_main_keyboard()
        )
        return 0
    ...
    # اگه اینجا رسیدیم و توی گروه بودیم => از قبل برگشتیم
    # ولی اگه توی پیوی بودیم => پیام راهنما
    if not is_private:
        return 0
...
def main():
    ...
    logger.info("ربات پیشرفته با سیستم سکوت در گروه شروع به کار کرد!")
