...
# ========== هوش مصنوعی با Gemini (اصلاح شده نهایی) ==========
async def chat_with_ai(user_id, question):
    if GEMINI_API_KEY:
        try:
            url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key={GEMINI_API_KEY}"
            payload = {"contents": [{"parts": [{"text": f"به فارسی جواب بده: {question}"}]}]}
    ...
    return "🔑 لطفاً GEMINI_API_KEY رو توی Railway تنظیم کن.\n\n(برای فعال کردن هوش مصنوعی)"
...
def main():
    ...
    logger.info("ربات با هوش مصنوعی شروع به کار کرد!")
