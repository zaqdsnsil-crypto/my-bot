...
# ========== سیستم افزایش بازدید پست (Views Booster) ==========
...
async def boost_post_views_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, text, user_id):
    ...
    if context.user_data.get(WAITING_FOR_POST_LINK):
        ...
        await update.message.reply_text("✅ لینک ذخیره شد!\n\n🔢 حالا تعداد بازدید رو وارد کن (حداکثر 5000، حداقل 10):")
        return True
    
    # مرحله 2: تعداد بازدید
    if context.user_data.get(WAITING_FOR_VIEWS_COUNT):
        try:
            ...
            if views_count < 10:
                await update.message.reply_text("❌ حداقل بازدید 10 هست!")
            elif views_count > 5000:
                await update.message.reply_text("❌ حداکثر بازدید 5000 هست!\nاگه بیشتر نیاز داری، چند بار جداگانه انجام بده.")
            else:
                # بررسی فرمت لینک
                if "t.me/" in post_link or "telegram.me/" in post_link:
                    await update.message.reply_text(
                        f"🎯 درخواست افزایش {views_count} بازدید برای پست ثبت شد!\n\n"
                        f"📊 وضعیت: در صف پردازش\n⏱️ زمان تقریبی: چند ثانیه\n\n"
                        f"✅ به زودی اطلاع داده میشه."
                    )
                    ...
                    add_points(user_id, 5, "view_request")
                    
                    await update.message.reply_text(
                        f"✅ درخواست افزایش بازدید با موفقیت ثبت شد!\n\n"
                        f"🔗 لینک: {post_link}\n"
                    ...
                    await update.message.reply_text("❌ فرمت لینک اشتباهه!\nلینک باید مثل `t.me/ChannelName/123` باشه.")
        except ValueError:
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "📈 افزایش بازدید":
        ...
        await update.message.reply_text(
            "🔗 *افزایش بازدید پست*\n\n"
            "لینک پست رو بفرست:\n"
            "مثال: `https://t.me/MyChannel/5`\n\n"
            "⚠️ حداکثر 5000 بازدید در هر بار\n"
            "✅ حداقل 10 بازدید",
            parse_mode="Markdown", reply_markup=get_main_keyboard()
        )
        return
    
    # بررسی مرحله افزایش بازدید
...
def main():
    ...
    logger.info("✅ ربات هوشمند با سیستم افزایش بازدید روشن شد!")
    app.run_polling()
