...
# ==================== پاک کردن منوی قدیمی Reply Keyboard ====================
async def remove_old_keyboard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """پاک کردن منوی قدیمی که از کد قبلی مانده"""
    try:
        msg = await update.message.reply_text(
            text="✅ منوی قدیمی پاک شد.\n\nحالا می‌تونی راحت چت کنی 😊",
            reply_markup=ReplyKeyboardRemove()
        )
        # پیام را بعد از ۴ ثانیه حذف می‌کنیم تا گروه تمیز بماند
        await asyncio.sleep(4)
        await msg.delete()
    except Exception as e:
        logger.error(f"خطا در پاک کردن کیبورد: {e}")
...
def main():
    ...
    # هندلر پاک کردن منوی قدیمی
    app.add_handler(CommandHandler(["clean", "remove", "fix", "clear"], remove_old_keyboard))
    
    logger.info("✅ ربات با موفقیت راه‌اندازی شد!")
    print("✅ ربات روشن شد!")
    print("📌 دستورات مفید: /clean  یا  /remove  برای پاک کردن منوی قدیمی")
